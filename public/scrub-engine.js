...
function mountScrollWorld(container, config) {
  ...
  SECTIONS.forEach((s, i) => {
    ...
    dot.type = 'button';
    dot.setAttribute('aria-label', `Jump to ${s.label || `section ${i + 1}`}`);
  ...
  // Document-relative top of the container. Segment start/end are container-relative
  // (0-based), but window.scrollY is page-absolute — the container isn't necessarily
  // at the top of the page (it can sit after a hero, etc.), so every read of scrollY
  // must be corrected by this offset or the timeline runs ahead of where the container
  // actually is. getBoundingClientRect().top + scrollY is scroll-position-invariant,
  // so it's safe to recompute any time (including mid-scroll, on resize).
  let origin = 0;
  ...
  function read() {
    const y = (window.scrollY || window.pageYOffset) - origin;
    ...
    for (let i = 0; i < N; i++) {
      ...
      let cop;
      if (i === 0) {
        // Fades in over the leading cushion as the container is approached (y < 0,
        // since the container can sit below other page content), holds briefly on
        // arrival, then fades per the original "greets on landing" curve.
        if (after) cop = 0;
        else if (before) cop = smooth(1 - (seg.start - y) / vh);
        else cop = smooth(1 - pr / 0.62);
      }
      else if (i === N - 1) {
        // Holds the CTA through the section, then fades it out over the track's
        // trailing cushion vh (see track.style.height in layout()) instead of
        // staying at opacity 1 forever — sw-copylayer is position:fixed, so a
        // stuck-open last panel would sit on top of every section below the world.
        if (before) cop = 0;
        else if (after) cop = smooth(1 - (y - seg.end) / vh);
    ...
    // Belt-and-suspenders: once scroll is a full cushion clear of either end of the
    // track, fully disable the whole fixed overlay (route dots, hint, topbar,
    // scrollbar included) rather than relying on every layer's own fade math to
    // reach exactly zero — position:fixed means anything left even faintly on would
    // sit on top of whatever page content precedes or follows the world section.
    container.classList.toggle('sw-idle', y < -vh || y > SEGMENTS[NSEG - 1].end + vh);
    ...
    // Fades in on approach (y < 0 while the container is still below other page
    // content) and out shortly after arrival — previously pinned at opacity 1 for
    // any y <= 0, which is only ever true when nothing precedes the world section.
    hint.style.opacity = y < 0 ? clamp(1 - (-y) / vh) : clamp(1 - y / (0.5 * vh));
  ...
  // Arrow-key section switching, like a tab strip — Right/Down for next,
  // Left/Up for previous. Scoped to only fire while the world is actually
  // engaged (container not sw-idle) so it never hijacks normal arrow-key page
  // scrolling before/after the world section, and never fires while the user
  // is typing into a form field.
  window.addEventListener('keydown', (e) => {
    if (container.classList.contains('sw-idle')) return;
    ...
    if (!dir) return;
    const next = clamp(activeIndex + dir, 0, N - 1);
...
function injectCSS() {
  ...
  .sw-root.sw-idle .sw-sky,.sw-root.sw-idle .sw-stage,.sw-root.sw-idle .sw-copylayer,.sw-root.sw-idle .sw-route,.sw-root.sw-idle .sw-hint,.sw-root.sw-idle .sw-topbar,.sw-root.sw-idle .sw-scrollbar{opacity:0;pointer-events:none;}
  `;
